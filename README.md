...
### (Stage 1) Initial Baseline: Multi-Layer Perceptron (MLP)
...
#### Conclusion:
* The model immediately prioritizes memorization over generalization once trained for more than a few passes, since the single-subject dataset (90 samples) is far too small relative to ~657,000 learnable parameters in W1 alone.
* Though, a test accuracy of ~45% is not insignificant, as it demonstrates the signal is actually learnable, but the architecture has no way to exploit the spatial/temporal structure of EEG (it flattens all 64 channels × timepoints into one undifferentiated vector).
* This motivates building a more sophisticated model, a CNN.

### (Stage 2-3) Convolutional Neural Network (CNN)
...
*The next step is implementing EEGNet (Lawhern et al., 2018), a compact CNN architecture purpose-built for EEG signals. 

### Key Differences:
* **Depthwise convolutions**: learn spatial filters per-channel rather than mixing all channels immediately
*  **Separable convolutions**: factor temporal and channel-mixing operations apart, significantly reducing parameter count relative to standard Conv2d stacking
